...
@Component
public class AddUsersInServiceAgreementHandler extends
    LeanGenericEventEmitter<SingleParameterHolder<String>, List<UsersDto>, Void> {

    private static final Logger LOGGER = LoggerFactory.getLogger(AddUsersInServiceAgreementHandler.class);

    private PersistenceServiceAgreementService persistenceServiceAgreementService;
    private ServiceAgreementBusinessRulesService serviceAgreementBusinessRulesService;

    public AddUsersInServiceAgreementHandler(EventBus eventBus,
     ...
     */
    @Override
    protected Void executeRequest(SingleParameterHolder<String> parameterHolder, List<UsersDto> requestData) {
        String serviceAgreementId = parameterHolder.getParameter();
        LOGGER.info("Trying to add participant's users for Service Agreement {} ", serviceAgreementId);
        ...
        persistenceServiceAgreementService.addUsersInServiceAgreement(serviceAgreementId, requestData);
        return null;
    }
     ...
     */
    @Override
    protected Event createSuccessEvent(SingleParameterHolder<String> parameterHolder, List<UsersDto> request,
        Void response) {
        return null;
    }
