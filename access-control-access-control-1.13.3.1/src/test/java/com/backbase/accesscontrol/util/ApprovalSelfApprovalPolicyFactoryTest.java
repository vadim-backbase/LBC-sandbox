...
class ApprovalSelfApprovalPolicyFactoryTest {
    ...
    @InjectMocks
    private ApprovalSelfApprovalPolicyFactory approvalPolicyFactory;

    @Test
    void shouldCreateApprovalSelfApprovalPolicy() {
        ...
        ApprovalSelfApprovalPolicy approvalPolicy = approvalPolicyFactory.createPolicy("fgId", selfApprovalPolicy);

        assertThat(approvalPolicy.isCanSelfApprove(), is(true));
        assertThat(approvalPolicy.getFunctionGroupItem().getFunctionGroupItemId(), equalTo(functionGroupItemId));
        assertThat(approvalPolicy.getApprovalSelfApprovalPolicyBounds(), hasSize(1));

        ApprovalSelfApprovalPolicyBound approvalPolicyBound = approvalPolicy.getApprovalSelfApprovalPolicyBounds()
            .iterator().next();

        assertThat(approvalPolicyBound.getCurrencyCode(), equalTo("EUR"));
        assertThat(approvalPolicyBound.getUpperBound(), equalTo(BigDecimal.TEN));
    }
    ...
    void shouldThrowExceptionWhenBusinessFunctionWithPrivilegeNotExist() {
        ...
        SelfApprovalPolicyFactoryException exception = assertThrows(SelfApprovalPolicyFactoryException.class,
            () -> approvalPolicyFactory.createPolicy("fgId", selfApprovalPolicy));

        assertThat(exception.getMessage(), equalTo("Unable to create ApprovalSelfApprovalPolicy"));
    }
